...
export default function TestPage() {
  ...
  useEffect(() => {
    const w = window as unknown as { MathJax?: { typesetPromise?: () => Promise<void>; startup?: { promise: Promise<void> } } }
  ...
  const score = (() => {
    ...
    return { correct, total: totalSelectable, percent: totalSelectable ? Math.round((correct / totalSelectable) * 100) : 0 }
  })()
        ...
        <div className="card p-4 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-4">
            <div className={cn(
              ...
              !submitted && !revealAnswers ? 'border-gray-200 text-text-muted' :
              score.percent >= 80 ? 'border-green-400 text-green-700' :
              score.percent >= 60 ? 'border-amber-400 text-amber-700' :
...
function QuestionCard({
  ...
  const cardBorder = showFeedback && question.type === 'single_choice'
    ? isChoiceCorrect ? 'border-green-300' : chosen ? 'border-red-300' : 'border-border'
    : question.type === 'open_response' ? 'border-dashed border-border' : 'border-border'
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <span className={cn(
          ...
          question.type === 'open_response'
            ...
            : chosen ? 'bg-primary-light text-primary' : 'bg-gray-100 text-text-muted'
        )}>
          {question.type === 'open_response'
            ? `Свободен отговор`
            : chosen ? `Избрано: ${chosen}` : 'Неотговорен'}
        </span>
      </div>
                ...
                <div key={label} className="grid gap-1.5">
                  ...
                  <textarea
                    ...
                    className={cn(
                      ...
                      isCorrect ? 'border-green-400 bg-green-50 focus:ring-green-200' :
                      isIncorrect ? 'border-red-300 bg-red-50 focus:ring-red-200' :
  ...
  if (question.type === 'single_choice') {
    ...
    return (
      <div className={cn(
        ...
        {correct ? `Отлично. Верен отговор: ${question.correct_option}.` : `Верен отговор: ${question.correct_option}.`}
      </div>
    )
  }
