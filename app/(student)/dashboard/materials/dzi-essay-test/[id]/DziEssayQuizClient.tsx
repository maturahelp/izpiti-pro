...
import type { DziEssayMaterial, DziEssayQuizQuestion } from '@/data/dziEssayMaterials'
import { cn } from '@/lib/utils'

/** Only the fields the quiz needs; the theory sections stay on the server. */
export type DziEssayQuizClientMaterial = Pick<DziEssayMaterial, 'id' | 'group' | 'title' | 'quiz'>
...
/**
 * Presentational quiz. Entitlement is checked by the Server Component page
 * before this ever renders, so no client-side premium check is needed and the
 * essay materials dataset is never imported here.
 */
export function DziEssayQuizClient({ material }: { material: DziEssayQuizClientMaterial }) {
  const router = useRouter()

  const [questions, setQuestions] = useState<DziEssayQuizQuestion[]>(() =>
    material.quiz.map(shuffleQuestion)
  )
