...
/**
 * Presentational quiz. Receives the (already entitlement-checked) topic from
 * the Server Component page; it never imports the question bank itself.
 */
export function RuleQuizClient({
  id,
  sectionTitle,
  topic,
}: {
  ...
  useEffect(() => {
    setShuffledQuestions(topic.questions.map(shuffleQuestion))
    setCurrentIndex(0)
    setSelectedOpt(null)
    setChecked(false)
    setAnswers({})
