...
function readStoredTestState(storageKey: string | null): {
  ...
  try {
    ...
    return {
      ...
      openResponses: saved.openResponses || {},
      submitted: Boolean(saved.submitted),
      revealAnswers: Boolean(saved.revealAnswers),
    }
  } catch {
    return {
      answers: {},
...
// Page component
// ---------------------------------------------------------------------------
export default function TestPageClient({ testId }: { testId: string }) {
  ...
  const exam = datasetId
    ? [...OFFICIAL_EXAMS, ...MOCK_EXAMS, ...BERON_EXAMS].find((e) => e.id === datasetId) ?? null
    : null
  const storageKey = test ? `izpiti-pro:test:${test.id}:state:v1` : null

  const [answers, setAnswers] = useState<SingleChoiceAnswers>(() => readStoredTestState(storageKey).answers)
  const [openResponses, setOpenResponses] = useState<OpenResponses>(() => readStoredTestState(storageKey).openResponses)
  const [submitted, setSubmitted] = useState(() => readStoredTestState(storageKey).submitted)
  const [revealAnswers, setRevealAnswers] = useState(() => readStoredTestState(storageKey).revealAnswers)
